// Genera con ElevenLabs (voz Alice, offline) el banco de voz de las SIMULACIONES ricas
// (config.fuente='sim-rica'): intro de cada mundo + instrucción + enunciado del reto/pregunta.
// NUNCA TTS. Salida: frontend/public/audio/sim-bank/<slug>.mp3 (idempotente; --force regenera).
import pg from 'pg';
...
const OUT_DIR = resolve(__dirname, '../../frontend/public/audio/sim-bank');
...
async function tts(text, outPath) {
  ...
  const buf = Buffer.from(await res.arrayBuffer()); writeFileSync(outPath, buf); return buf.length;
}

async function main() {
  if (!existsSync(OUT_DIR)) mkdirSync(OUT_DIR, { recursive: true });
  await client.connect();
  const rows = (await client.query("SELECT config FROM niveles WHERE config->>'fuente'='sim-rica'")).rows;
  const frases = new Map();
  for (const { config } of rows) {
    const add = (t) => { if (t && typeof t === 'string') frases.set(slugFrase(t), t); };
    add(config.narracion?.intro); add(config.instruccion);
    if (config.pregunta?.enunciado) add(config.pregunta.enunciado);
    if (config.reto?.enunciado) add(config.reto.enunciado);
  }
  ...
  await client.end();
  console.log(`\n✅ Banco sim-bank: ${ok} generados, ${skip} ya existían${fail ? `, ${fail} fallidos` : ''}.`);
}
